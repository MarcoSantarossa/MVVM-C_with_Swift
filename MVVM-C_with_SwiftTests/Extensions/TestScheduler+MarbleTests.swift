...
extension TestScheduler {
    ...
    */
    func createDriver<T>(timeline: String, values: [String: T], resolution: TimeInterval) -> Driver<T> {
        return createObservable(timeline: timeline, values: values, errors: [:], resolution: resolution).asDriver(onErrorRecover: { _ -> Driver<T> in
    ...
    */
    func createObservable<T>(timeline: String, values: [String: T], errors: [String: Swift.Error] = [:], resolution: TimeInterval) -> Observable<T> {
    ...
    /**
     ...
     - returns: Observable sequence specified by timeline and values.
    */
    func createObservable<T>(_ events: [Recorded<Event<T>>], resolution: TimeInterval) -> Observable<T> {
        return createObservable([events], resolution: resolution)
    }
    ...
    */
    ...
    /**
     ...
     - returns: Implementation of method that accepts arguments with parameter `Arg` and returns observable sequence
        with parameter `Ret`.
     */
    func mock<Arg, Ret>(values: [String: Ret], errors: [String: Swift.Error] = [:], resolution: TimeInterval, timelineSelector: @escaping (Arg) -> String) -> (Arg) -> Observable<Ret> {
